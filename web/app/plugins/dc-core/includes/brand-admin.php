 ...
 * overriding `--dc-*` tokens, not by editing selectors.
 *
 ...
 * `--dc-logo-url` / `--dc-favicon-url` custom properties. Until
...
{
    return DC_CORE_URL . 'assets/' . ltrim($path, '/');
}
...
function enqueue_admin_assets(): void
{
    ...
    wp_add_inline_style('dc-admin-brand', brand_asset_css());
}
...
function enqueue_login_assets(): void
{
    ...
    wp_add_inline_style('dc-login-brand', brand_asset_css());
}
 ...
 */
/**
 * Read per-instance branding from the `dc_brand` option (data, not a
 ...
 * to none. Colours map onto the login/admin stylesheet's --dc-* tokens.
 *
 * @return array<string, string>
 */
function brand_config(): array
{
    $opt = get_option('dc_brand', []);
...
function brand_logo_url(): string
{
    ...
    $logo_file = DC_CORE_DIR . 'assets/brand/logo.png';
...
{
    $brand        = brand_config();
    $favicon_file = DC_CORE_DIR . 'assets/brand/favicon-32x32.png';
    ...
    $vars = [
        '--dc-logo-url:' . ($logo_url ? 'url("' . esc_url($logo_url) . '")' : 'none'),
        '--dc-favicon-url:' . ($favicon_url ? 'url("' . esc_url($favicon_url) . '")' : 'none'),
    ];
    ...
    $colour_map = [
        'primary' => '--dc-brand-500',
        'accent'  => '--dc-accent-500',
        'ink'     => '--dc-ink-900',
    ];
...
{
    return get_bloginfo('name') ?: __('Decoupled CMS', 'dc-core');
}
...
{
    return __('Decoupled WordPress · Decoupled Core', 'dc-core');
}
