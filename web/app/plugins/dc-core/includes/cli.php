<?php
/**
 * WP-CLI commands for the Decoupled content model.
 *
 ...
 * `dc_content_model_active` option; dc-core interprets it at
 * runtime. No executable code ever comes from the model.
 *
 *   wp dc model export [--pretty] [> model.json]
 *   wp dc model validate <file.json|->
 *   wp dc model import <file.json|->        (validate + store + flush)
 *   wp dc model use <name>                  (a bundled models/<name>.json)
 *   wp dc model list                        (bundled model names)
 *   wp dc model reset                       (back to the built-in default)
 */

namespace Dc\Core\CLI;
...
\WP_CLI::add_command('dc model', __NAMESPACE__ . '\\Model_Command');
\WP_CLI::add_command('dc content', __NAMESPACE__ . '\\Content_Command');

/**
 * Manage the Decoupled content model as data (no PHP uploads).
 */
class Model_Command
{
    /**
    ...
    public function use($args): void
    {
        ...
        if (!is_file($path)) {
            \WP_CLI::error("No bundled model '{$name}'. Run `wp dc model list`.");
        }
 ...
 *
 * The model defines the shape (`wp dc model`); this loads actual
 * content against that shape from a JSON envelope. The WP analogue of
 * Drupal's dc_import.
 *
 *   wp dc content example [--pretty]      print a valid envelope
 *   wp dc content validate <file.json|->  validate, don't write
 *   wp dc content import <file.json|->     validate + import
 *   wp dc content seed [<name>]            import a bundled seed
 *   wp dc content seeds                    list bundled seeds
 */
class Content_Command
{
    ...
    public function example($args, $assoc): void
    {
        $flags = JSON_UNESCAPED_SLASHES
            | (isset($assoc['pretty']) ? JSON_PRETTY_PRINT : 0);
        \WP_CLI::line((string) wp_json_encode(\Dc\Core\Content\example_envelope(), $flags));
    }
    ...
    public function validate($args): void
    {
        ...
        $errors = \Dc\Core\Content\validate_envelope($envelope);
        if ($errors !== []) {
            foreach ($errors as $e) {
                \WP_CLI::warning($e);
            }
    ...
    {
        $envelope = $this->read_json($args[0]);
        $result = \Dc\Core\Content\import_envelope($envelope);
     ...
     *
     * The starter-content analogue of `wp dc model use`: a fresh
    ...
    public function seed($args): void
    {
        ...
        $path = DC_CORE_DIR . 'seed/' . $name . '.json';
        if (!is_file($path)) {
            \WP_CLI::error("No bundled seed '{$name}'. Run `wp dc content seeds`.");
        }
