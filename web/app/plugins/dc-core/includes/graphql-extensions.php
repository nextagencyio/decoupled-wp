 ...
 *
 * `DcComponent` is one COMPLEX COMPONENT (gallery / CTA / embed /
...
{
    register_graphql_object_type('DcImage', [
        'description' => __('An image reference: src + alt (+ optional dimensions).', 'dc-core'),
        'fields' => [
            'src'    => ['type' => 'String'],
            'alt'    => ['type' => 'String'],
            'width'  => ['type' => 'String'],
    ...
    register_graphql_object_type('DcTerm', [
        'description' => __('A taxonomy term reference: slug + display name.', 'dc-core'),
        'fields' => [
            'slug' => ['type' => 'String'],
            'name' => ['type' => 'String'],
        ],
    ...
    register_graphql_object_type('DcRef', [
        'description' => __('A reference to a related post: id + slug + title.', 'dc-core'),
        'fields' => [
            'id'    => ['type' => 'String'],
            'slug'  => ['type' => 'String'],
            'title' => ['type' => 'String'],
    ...
    register_graphql_object_type('DcBlock', [
        'description' => __('A normalized content block from a kind-discriminated complex field (body block / component / page section). `kind` selects which slots are meaningful; `data` is the full row as JSON for anything the typed slots miss.', 'dc-core'),
        'fields' => [
            'kind'        => ['type' => 'String'],
            'html'        => ['type' => 'String'],
            'text'        => ['type' => 'String'],
    ...
    register_graphql_object_type('DcComponent', [
        ...
        'fields' => [
            ...
            'images'      => ['type' => ['list_of' => 'DcImage'], 'description' => 'Images (gallery components).'],
...
/** Resolve the optional components Complex into the DcComponent list. */
function components_resolver(string $key = 'dc_components'): callable
{
    return function ($post) use ($key) {
        $raw = carbon_get_post_meta($post->databaseId, $key);
        // Generic {kind, props-JSON} for the full palette.
        $blocks = dc_components_to_blocks($raw);
        ...
        $legacy = dc_components_to_array($raw);
...
function intro_resolver(string $key = 'dc_intro_paragraphs'): callable
{
    return function ($post) use ($key) {
        $raw = carbon_get_post_meta($post->databaseId, $key);
        return dc_intro_to_strings($raw);
    };
}

/** Resolve a stored gallery Complex into a list of DcImage. */
function gallery_resolver(string $key = 'dc_gallery_images'): callable
{
    return function ($post) use ($key) {
        $raw = carbon_get_post_meta($post->databaseId, $key);
        return dc_gallery_to_images($raw);
    };
}

/**
 * Resolve a hero image (url meta) + its alt into a single DcImage,
 * or null when no hero image is set.
 */
function hero_resolver(string $src_key = 'dc_hero_image', string $alt_key = 'dc_hero_alt'): callable
{
 ...
 * the queries in astro's src/lib/wp.ts.
 */
function register_shared_fields(string $graphql_type): void
{
    ...
    foreach ($fields as $field_name => $field_config) {
        ...
        register_graphql_field($graphql_type, (string) $field_name, [
            ...
            'description' => __((string) ($field_config['description'] ?? ''), 'dc-core'),
            'resolve'     => $resolver,
        ]);
    }
}
...
function resolver_from_config(array $config): ?callable
{
    return match ((string) ($config['resolver'] ?? '')) {
        'heroImage'       => hero_resolver((string) ($config['srcKey'] ?? 'dc_hero_image'), (string) ($config['altKey'] ?? 'dc_hero_alt')),
        'introParagraphs' => intro_resolver((string) ($config['key'] ?? 'dc_intro_paragraphs')),
        'bodyHtml'        => body_html_resolver(),
        'components'      => components_resolver((string) ($config['key'] ?? 'dc_components')),
        'galleryImages'   => gallery_resolver((string) ($config['key'] ?? 'dc_gallery_images')),
...
/**
 * Resolve a taxonomy's terms on a post into a list of DcTerm
 ...
 * DcRef (id + slug + title), so the frontend can render relations
 ...
 * (the complex group name). This maps each row into a flat DcBlock:
...
/**
 * Resolve one DcBlock slot from a complex row, per its config spec.
...
/**
 * Fields on the example `dc_resource` CPT (GraphQL type `Resource`).
 */
