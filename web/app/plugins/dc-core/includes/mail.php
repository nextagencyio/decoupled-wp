<?php
/**
 * Decoupled Core — route wp_mail() through the Resend HTTP API.
 *
...
function pre_send($short_circuit, array $atts)
{
    ...
    if (is_wp_error($res)) {
        error_log('[dc_mail] Resend transport error: ' . $res->get_error_message());
        // Fire wp_mail_failed for parity with the default path.
        do_action('wp_mail_failed', new \WP_Error('dc_mail_resend_failed', $res->get_error_message(), $atts));
        return false;
    }
    ...
    if ($code < 200 || $code >= 300) {
        ...
        error_log("[dc_mail] Resend HTTP {$code}: " . substr($body_text, 0, 500));
        do_action('wp_mail_failed', new \WP_Error('dc_mail_resend_http', "Resend HTTP {$code}: {$body_text}", $atts));
        return false;
    }
