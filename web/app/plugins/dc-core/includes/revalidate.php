 ...
 *
 * Config:
 *   - DC_FRONTEND_URL              frontend origin
 *   - DC_REVALIDATE_BYPASS_TOKEN   shared bypass token
 */

namespace Dc\Core\Revalidate;
...
{
    $frontend = defined('DC_FRONTEND_URL')
        ? DC_FRONTEND_URL
        : (getenv('DC_FRONTEND_URL') ?: 'http://localhost:4321');
    $frontend = rtrim($frontend, '/');

    $token = defined('DC_REVALIDATE_BYPASS_TOKEN')
        ? DC_REVALIDATE_BYPASS_TOKEN
        : (getenv('DC_REVALIDATE_BYPASS_TOKEN') ?: '');
