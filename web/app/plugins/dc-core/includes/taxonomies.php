 ...
 *
 * Ships ONE generic example taxonomy — `dc_topic` — attached to the
 * example `dc_resource` CPT and the default `post`. It demonstrates
...
function register_topic(): void
{
    ...
    if (!isset($taxonomies['dc_topic'])) {
        return;
    }

    $config = $taxonomies['dc_topic'];
    $object_type = is_array($config['object_type'] ?? null) ? $config['object_type'] : [];
    $args = is_array($config['args'] ?? null) ? $config['args'] : [];
    register_taxonomy('dc_topic', $object_type, $args);
}
