 ...
 * short-lived edit token via the dc-puck REST route and redirects to
 ...
 * option, then the DC_FRONTEND_URL env the dev stack already sets,
 * then a sensible local default.
 */
function editor_base_url(): string
{
    $opt = (string) get_option('dc_puck_editor_url', '');
    if ($opt !== '') {
        return rtrim($opt, '/');
    }
    $env = (string) (getenv('DC_FRONTEND_URL') ?: '');
    if ($env !== '') {
        return rtrim($env, '/');
    }
...
function enabled_post_types(): array
{
    ...
    if (!function_exists('Dc\Core\\Config\\field_groups')) {
        return $types;
    }
    foreach (\Dc\Core\Config\field_groups() as $group) {
        if (!is_array($group)) {
            continue;
        }
...
function render_edit_screen_button(\WP_Post $post): void
{
    ...
    printf(
        '<p style="margin:12px 0;"><a href="#" class="button button-primary button-hero dc-puck-launch" data-puck-id="%d">%s</a></p>',
        (int) $post->ID,
        esc_html__('Open Design Studio', 'dc-puck')
    );
}
...
function admin_bar_button(\WP_Admin_Bar $bar): void
{
    ...
    $bar->add_node([
        ...
        'id'    => 'dc-puck-launch-' . $post_id,
        'title' => esc_html__('Open Design Studio', 'dc-puck'),
        'href'  => '#',
        'meta'  => ['class' => 'dc-puck-launch'],
    ]);
}
...
function print_launch_script(): void
{
    ...
    $rest_root = esc_url_raw(rest_url('dc/v1/token/'));
    ...
    (function () {
      ...
      document.addEventListener('click', function (e) {
        var el = e.target.closest('.dc-puck-launch');
        if (!el) return;
        e.preventDefault();
        // Edit-screen button carries data-puck-id; the admin-bar item
        // carries it in its <li id="wp-admin-bar-dc-puck-launch-{id}">.
        var id = el.getAttribute('data-puck-id');
        if (!id) {
          var host = el.closest('[id*="dc-puck-launch-"]') || el;
          var m = (host.id || '').match(/dc-puck-launch-(\d+)/);
          id = m ? m[1] : null;
        }
        if (!id) return;
