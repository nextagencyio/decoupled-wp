 ...
 *
 * Mirrors dc_puck's routes under the dc/v1 namespace:
...
namespace Dc\Puck\Rest;

use Dc\Puck\Mapping;
use Dc\Puck\Token;
use Dc\Puck\Transform;
...
function configure(\WP_REST_Request $request): \WP_REST_Response
{
    ...
    if (isset($body['sectionsField']) && is_string($body['sectionsField'])) {
        update_option('dc_puck_sections_field', sanitize_key($body['sectionsField']));
    }
    return new \WP_REST_Response([
        'ok'            => true,
        'sectionsField' => Mapping\sections_field(),
