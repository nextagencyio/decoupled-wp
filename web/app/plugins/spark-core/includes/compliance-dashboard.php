 ...
 * `<SPARK_AUDIT_URL>/audits/<scanner>/latest.json`, distills it into
...
namespace Spark\Core\ComplianceDashboard;
...
const MENU_SLUG = 'spark-compliance';
const SCANNERS  = ['a11y', 'seo', 'performance'];

/**
 * Where audit JSON lives. Defaults to the WordPress home URL; override
 * with SPARK_AUDIT_URL (define or env var) to point at the deployed
 * Astro frontend, which is where CI publishes the audit JSON.
 */
function audit_base(): string
{
    $base = defined('SPARK_AUDIT_URL')
        ? SPARK_AUDIT_URL
        : (getenv('SPARK_AUDIT_URL') ?: default_audit_url());
    return rtrim($base, '/') . '/audits';
}

/**
 * Fallback audit origin when SPARK_AUDIT_URL is not configured — the
...
function default_audit_url(): string
{
    ...
    return getenv('SPARK_FRONTEND_URL') ?: home_url('/');
}
...
{
    add_menu_page(
        __('Compliance Dashboard', 'spark-core'),
        __('Compliance', 'spark-core'),
        CAP,
        MENU_SLUG,
...
function enqueue_assets(string $hook): void
{
    ...
    wp_enqueue_style(
        'spark-compliance-dashboard',
        SPARK_CORE_URL . 'assets/css/compliance-dashboard.css',
        [],
        SPARK_CORE_VERSION
    );
}
...
{
    $cache_key = 'spark_compliance_' . $scanner;
...
function maybe_refresh(): void
{
    ...
    check_admin_referer('spark_compliance_refresh');
    foreach (SCANNERS as $s) {
        delete_transient('spark_compliance_' . $s);
    }
...
function render_page(): void
{
    if (!current_user_can(CAP)) {
        wp_die(esc_html__('Insufficient permissions.', 'spark-core'));
    }
    ...
    // the intended production URL via SPARK_AUDIT_LABEL — so the
    // dashboard reads cleanly without exposing a docker/internal host.
    $base_label = defined('SPARK_AUDIT_LABEL')
        ? SPARK_AUDIT_LABEL
        : (getenv('SPARK_AUDIT_LABEL') ?: $base);

    $refresh_url = wp_nonce_url(
        admin_url('admin.php?page=' . MENU_SLUG . '&refresh=1'),
        'spark_compliance_refresh'
    );

    include SPARK_CORE_DIR . 'templates/compliance-dashboard.php';
}
