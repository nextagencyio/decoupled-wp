 ...
 * directory. Selecting one with SPARK_CONTENT_MODEL (env or constant)
 ...
 *   1. stored option (admin screen / `wp spark model import`)
 *   2. this filter (SPARK_CONTENT_MODEL bundled JSON)
...
namespace Spark\Core\Config\Bundled;
...
add_filter('spark_core_content_model', static function ($model) {
    // SPARK_CONTENT_MODEL is the PROVISIONING DEFAULT for a fresh
    // instance — it must NOT override a model an operator explicitly
    // stored (admin screen / `wp spark model import|use`). If a stored
    // option exists, that wins; the env default only fills the empty case.
    if (is_array(\Spark\Core\Config\stored_model())) {
        return $model;
    }
    $selected = defined('SPARK_CONTENT_MODEL')
        ? SPARK_CONTENT_MODEL
        : (getenv('SPARK_CONTENT_MODEL') ?: '');
    if ($selected === '') {
        return $model;
    }
