...
/**
 * Return the active Spark content model.
 *
...
function model(bool $refresh = false): array
{
    ...
    $candidate = apply_filters('spark_core_content_model', $candidate);
    if (!is_array($candidate)) {
        $candidate = default_model();
    }
...
{
    $stored = get_option('spark_content_model_active', null);
...
function save_model(array $candidate): array
{
    ...
    update_option('spark_content_model_active', $candidate, false);
...
{
    delete_option('spark_content_model_active');
    model(true);
    flush_rewrite_rules();
}
...
function allowed_graphql_resolvers(): array
{
    return [
        ...
        // `terms` = a taxonomy's terms ([SparkTerm]); `relatedPosts` =
        // an association value ([SparkRef]).
        'meta', 'metaComplexJson', 'metaInt', 'metaBool', 'metaList',
        ...
        // variants + per-kind slot map; output is [SparkBlock].
        'variantMap',
    ];
}
...
{
    $GLOBALS['spark_core_content_model_errors'] = $errors;
}
...
{
    return $GLOBALS['spark_core_content_model_errors'] ?? [];
}
