...
namespace Spark\Core\Dashboard;
...
function register_widgets(): void
{
    wp_add_dashboard_widget(
        'spark_decoupled_status',
        __('Decoupled Status', 'spark-core'),
...
{
    $url = defined('SPARK_FRONTEND_URL')
        ? SPARK_FRONTEND_URL
...
function render_decoupled_status(): void
{
    ...
    ?>
    <p>
        <strong><?php esc_html_e('Public site:', 'spark-core'); ?></strong>
        <a href="<?php echo esc_url($frontend); ?>" target="_blank" rel="noopener">
            <?php echo esc_html($frontend); ?>
        </a>
    </p>
    <p>
        <strong><?php esc_html_e('Stack:', 'spark-core'); ?></strong>
        <?php esc_html_e('WordPress (headless CMS) + WPGraphQL + Astro (public frontend)', 'spark-core'); ?>
    </p>
    <p>
        <?php esc_html_e('This WordPress install is the editorial backend. The public site renders from Astro. Publishing or updating a post fires an on-demand revalidation so the frontend rebuilds just the affected pages.', 'spark-core'); ?>
    </p>
    <?php
}
