 ...
 *   2. `spark_resource` — the example CPT additionally gets the
...
add_action('carbon_fields_register_fields', function () {
    ...
    foreach (Config\field_groups() as $group) {
        spark_register_configured_field_group($group);
    }
});
 ...
 */
function spark_register_configured_field_group(array $group): void
{
    ...
    $container = Container::make(
        'post_meta',
        __((string) ($group['label'] ?? 'Content details'), 'spark-core')
    ...
    foreach ($tabs as $tab) {
        ...
        foreach ($field_configs as $field_config) {
            ...
            $field = spark_make_configured_field($field_config);
            if ($field) {
                $fields[] = $field;
            }
        }
        ...
        if ($fields !== []) {
            $container->add_tab(__((string) ($tab['label'] ?? 'Content'), 'spark-core'), $fields);
        }
    }
}
 ...
 */
function spark_make_configured_field(array $config)
{
    $type = (string) ($config['type'] ?? '');
    $key = (string) ($config['key'] ?? '');
    $label = __((string) ($config['label'] ?? spark_humanize_field_key($key)), 'spark-core');
    ...
    if ($type === 'preset') {
        $field = spark_make_preset_field((string) ($config['preset'] ?? ''), $key);
        return $field ? spark_apply_field_options($field, $config) : null;
    }

    if ($type === 'complex') {
        $field = spark_make_complex_field($key, $label, $config);
        return $field ? spark_apply_field_options($field, $config) : null;
    }

    if ($type === 'association') {
        $field = spark_make_association_field($key, $label, $config);
        return $field ? spark_apply_field_options($field, $config) : null;
    }
    ...
    return spark_apply_field_options($field, $config);
}
 ...
 */
function spark_make_complex_field(string $key, string $label, array $config)
{
    ...
    foreach ($sub_configs as $sub) {
        ...
        $sub_label = __((string) ($sub['label'] ?? spark_humanize_field_key($sub_key)), 'spark-core');
        ...
        $sub_fields[] = spark_apply_field_options($sub_field, $sub);
    }
 ...
 * @return mixed Carbon Fields field instance, or null when unsupported.
 */
function spark_make_preset_field(string $preset, string $key)
{
    return match ($preset) {
        'introParagraphs' => spark_make_intro_field($key),
        'components'      => spark_make_components_field($key),
        'galleryImages'   => spark_make_gallery_field($key),
        default           => null,
    };
}
 ...
 */
function spark_make_association_field(string $key, string $label, array $config)
{
 ...
 * @return mixed
 */
function spark_apply_field_options($field, array $config)
{
    ...
    if (isset($config['help_text']) && method_exists($field, 'set_help_text')) {
        $field->set_help_text(__((string) $config['help_text'], 'spark-core'));
    }
...
function spark_humanize_field_key(string $key): string
{
    $label = preg_replace('/^spark_/', '', $key);
